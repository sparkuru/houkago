...
test("部長 SHINKOU is broadcast to 部員 in the same room", async () => {
  const room = await makeAuthenticatedRoom(baseHttp, "sync_host_one")
  const host = await openAuthenticatedSocket(baseHttp, baseWs, room.id, "sync_host_one")
...
test("late joiner catches up: OIKAKE → GENJOU reflects the 部長's last Shinkou", async () => {
  const room = await makeAuthenticatedRoom(baseHttp, "sync_host_two")
  const host = await openAuthenticatedSocket(baseHttp, baseWs, room.id, "sync_host_two")
  const roomId = room.id
  ...
  const late = await openAuthenticatedSocket(baseHttp, baseWs, roomId, "late")
...
test("non-部長 SHINKOU is rejected with KEIHOU and not broadcast", async () => {
  const room = await makeAuthenticatedRoom(baseHttp, "sync_host_three")
  const host = await openAuthenticatedSocket(baseHttp, baseWs, room.id, "sync_host_three")
  const roomId = room.id
  const intruder = await openAuthenticatedSocket(baseHttp, baseWs, roomId, "viewer")
