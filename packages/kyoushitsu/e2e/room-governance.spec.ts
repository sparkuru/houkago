...
test("control policy presets converge for members and retain owner-only queue management", async ({
  ...
  try {
    ...
    await joinRoom(memberPage, roomUrl)
    await openBangumi(memberPage)
    const connectionManager = memberPage.getByRole("button", { name: "管理百度连接" })
    await expect(connectionManager).toBeVisible()
    await expect(memberPage.getByRole("button", { name: "从百度网盘选择" })).toHaveCount(0)
    await expect(memberPage.getByRole("button", { name: "添加链接" })).toHaveCount(0)
    await connectionManager.click()
    const connectionDialog = memberPage.getByRole("dialog", { name: "连接百度网盘" })
