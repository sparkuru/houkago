...
| 身份与授权 | 已完成 | 自托管账号、可撤销会话、服务器派生的 REST/WS 身份与队列授权已落地；旧 UUID 所有权会明确要求重置而不被迁移 |

**当前主线：房间治理基础。** 在已完成的服务器身份与部室授权之上，建立持久成员关系、
房主可见的成员名册和安全移除边界；现有成员可跨重连恢复，新的入室决定仍由当前模式控制。
它不批准任何新的第三方平台或来源会话能力。

推荐的交付顺序：

1. **持久成员与房间治理**：定义成员的重连/移除生命周期、房主可见名册和授权边界；
   不引入协管、逐成员权限或封禁系统。
2. **房间治理后续**：在不改变 URL-first 边界的前提下，按单独需求评估队列
   管理、字幕/音轨 UI 或控制权策略。
