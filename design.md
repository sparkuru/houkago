...
| P2 解析+代理 | 部分完成 | `Enmoku` 类型预留 `headers`/`subtitles`/`sources`/`danmaku`；`houkago-eisha` 包已建立；通用直链/HLS/DASH resolver；base64url 稳定代理 token；housou 挂载 `/eisha/proxy/:token`；基础 Range/seek 代理测试；m3u8 manifest URI/segment 重写 | 无平台解析器；无过期 URL 续期；无浏览/搜索 UI；前端 dev 表单尚未接 resolver |
...
- 已完成：新建 `houkago-eisha` 包；通用直链 / m3u8 / mpd resolver；稳定代理 token；housou co-deploy 挂载 `/eisha/proxy/:token`；range/seek 基础行为测试；m3u8 manifest segment/key/map/media URI 重写。
- 下一刀：将前端 dev 直链表单接入 resolver/create 流程，让手填 m3u8/mp4 统一走 eisha 稳定代理地址。
