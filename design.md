...
| P2 解析+代理 | 部分完成 | `Enmoku` 类型预留并持久化 `headers`/`subtitles`/`sources`/`danmaku`/`live`；`houkago-eisha` 包已建立；通用直链/HLS/DASH resolver；Generic HLS manifest parser 可从 master/media playlist 产出 `sources`/`subtitles`/`live`；base64url 稳定代理 token；housou 挂载 `/eisha/proxy/:token`；基础 Range/seek 代理测试；m3u8 manifest URI/segment 重写；前端 dev 直链表单经 resolver 创建稳定代理演目 | 无站点平台解析器；无过期 URL 续期；无浏览/搜索 UI |
