...
| P2 解析+代理 | 部分完成 | `Enmoku` 类型预留并持久化 `headers`/`subtitles`/`sources`/`danmaku`/`live`；`houkago-eisha` 包已建立；通用直链/HLS/DASH resolver；base64url 稳定代理 token；housou 挂载 `/eisha/proxy/:token`；基础 Range/seek 代理测试；m3u8 manifest URI/segment 重写；前端 dev 直链表单经 resolver 创建稳定代理演目 | 无平台解析器；无过期 URL 续期；无浏览/搜索 UI |
...
- 已完成：新建 `houkago-eisha` 包；通用直链 / m3u8 / mpd resolver；稳定代理 token；housou co-deploy 挂载 `/eisha/proxy/:token`；range/seek 基础行为测试；m3u8 manifest segment/key/map/media URI 重写；前端 dev 直链表单接入 resolver/create 流程；`Enmoku.headers/subtitles/sources/danmaku/live` SQLite 持久化。
- 下一刀：加入过期 URL 自动重解析，或先做第一个平台解析器来产出真实 headers/subtitles/sources 元数据。
- 后续再加入平台浏览/搜索 API。
